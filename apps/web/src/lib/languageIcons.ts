...
const languageIconNamesById = new Map(
	languageIcons.flatMap(({ name, ids }) => ids.map((id) => [id.toLowerCase(), name] as const)),
);

export function getLanguageIconSrc(language: string): string | undefined {
	const iconName = languageIconNamesById.get(language.trim().toLowerCase());

	return iconName ? `/language-icons/${iconName}.svg` : undefined;
}
