...
/** Minimal tRPC error shape used by {@link PrintFunction.TRPCError}. */
export interface PrintTRPCError {
...
export interface PrintSetupOptions {
  /** Log prefix label. Defaults to `"PRINT"`. */
  prefix?: string;
  /** Prefix accent color (browser `%c`, Node chalk). Defaults to `#606060`. */
  color?: string;
}
...
/** Callable logger with styled channel helpers (also available as global `Print` after side-effect import). */
export type PrintFunction = {
  ...
  Setup: (options?: PrintSetupOptions) => void;
};
...
const createPrint = (() => {
  ...
  function printStyledLine(
    ...
    console[method](chalk.hex(prefixConfig.color).bold(`[${date}]`), ...parts);
  }
  ...
  function printDevLabeledLine(
    ...
    printStyledLine(
      method,
      date,
      chalk.hex(badge.color)(badge.text),
      ...formatted,
    );
  }
  ...
  function popPrintOptions(
    ...
    const popped =
      vals.length > 0 && isPrintOptions(vals[vals.length - 1])
        ? (vals.pop() as PrintOptions)
        : {};
    return {
  ...
  const printFunc = ((...val: any[]) => {
    ...
    const popped =
      val.length > 0 && isPrintOptions(val[val.length - 1])
        ? (val.pop() as PrintOptions)
        : {};
    ...
    if (
      isBrowser &&
      val.length >= 2 &&
      looksLikeConsoleFormatFirstArg(val[0])
    ) {
    ...
    if (active[title]) {
      ...
      if (elapsed >= minTime) {
        ...
        if (useProdJsonLogs) {
          ...
          printFunc(
            `${chalk.blue("[TIME]")} ${active[title]}: ${chalk.yellow(`${elapsed}ms`)}`,
          );
        }
      }
  ...
  printFunc.Extend = (
    ...
    const extendedPrint = (val: any[]) => {
      printStyledLine(
        method,
        formatPrintTime(),
        chalk.hex(color)(labelText),
        ...val
          .map((v) => chalk.hex(valueColor)(v.toString()))
          .concat(valueColorReset),
      );
    };
    return extendedPrint;
  };

  printFunc.Setup = (options: PrintSetupOptions = {}) => {
